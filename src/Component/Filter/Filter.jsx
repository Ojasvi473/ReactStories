import React from "react";
import "./Filter.css";
const Filter = () => {
  return (
    <>
      <h3 className="post">Lastest Posts</h3>
      <img
        className="m4"
        src="data:image/png;base64,"
        alt=""
        width="27px"
      />
      <h6 className="cat">Filter by Category</h6>
      <div className="filter">
        <button className="filter-option" id="all">
          All
        </button>
        <button className="filter-option">Artifical Intelligence</button>
        <button className="filter-option">Cloud Computing</button>
        <button className="filter-option">DevOps</button>
        <button className="filter-option">Programming Languages</button>

        <button className="filter-option">
          Mobile Application Development
        </button>
        <div className="filters">
          <button className="filter-option">Technology and Tools</button>
          <button className="filter-option">Get a Job in a Tech Company</button>
          <button className="filter-option">Others</button>
        </div>
      </div>
    </>
  );
};

export default Filter;
